...
define("arale/iframe-shim/1.0.2/iframe-shim", ["jquery", "arale/position/1.0.1/position" ], function($, Position) {
...
define("component/overlay/1.1.4/overlay", ["jquery", "arale/position/1.0.1/position", "arale/iframe-shim/1.0.2/iframe-shim", "arale/widget/1.1.1/widget"], function($, Position, Shim, Widget) {
    // Overlay
    // -------
    ...
    var ua = (window.navigator.userAgent || "").toLowerCase(), isIE6 = ua.indexOf("msie 6") !== -1, body = $(document.body), doc = $(document);
    // Mask
    // ----------
    // 全屏遮罩层组件
...
define("component/popup/1.0.0/popup", ["jquery", "bainx", "component/overlay/1.1.4/overlay", "arale/position/1.0.1/position"], function($, S, Overlay, Position) {
    ...
    var Popup = Overlay.extend({
        ...
        setup: function() {
            ...
            Popup.superclass.setup.call(instance);

            if (instance.get("withMask")) {
                instance._setupMask();
            }
        ...
        _setupKey: function() {
            ...
            instance.after("show", function() {
                if (!allPopups.length) {
                    // start to catch key event
                    $document.on("keydown", keyHandler);
                }
                allPopups.push(this);
            });

            instance.after("hide", erasePopup);
        },

        _setupMask: function() {
            var instance = this,
                maskConfig = instance.get("maskConfig");

            // popup element is in dom
            // insert mask before popup
            if (isInDocument(instance.element[0])) {
                maskConfig.relatedNode = instance.element;
                maskConfig.renderMethod = "insertBefore";
            }
            instance.mask = new Mask(maskConfig);
        },
        ...
        _trigger: function(type) {
            ...
            setTimeout(function() {
                instance.trigger("tbtx.popup." + type);
            }, 0);
        },
        ...
        show: function() {
            ...
            Popup.superclass.show.call(instance);
            instance._trigger("show");
        ...
        hide: function() {
            ...
            instance._trigger("hide");

            if (instance.get("autoDestroy")) {
                instance.destroy();
            }
            return instance;
        },
        ...
        destroy: function() {
            ...
            Popup.superclass.destroy.call(instance);
    ...
    // 需要处理键盘事件的popup
    var allPopups = [],
        erasePopup = function(instance) {
            erase(instance || this, allPopups);
            if (!allPopups.length) {
