 ...
 * @since 2013.8.2
 */
define("plugin/slider/1.0.0/slider2", ["jquery"], function() {
    ;(function(win, lib){
        ...
        $.extend(TouchSlider.prototype,{
            ...
            loadImg : function(n){  //判断加载哪屏图片
                ...
                var status = this.visible,
                    ...
                    panels = this.panel.children(); //this.panels;
                if(!this.loop){
                    start = Math.max(0, start);
                    end = Math.min(end, panels.length);
                }
                //console.log('n='+n+',start='+start+', end='+end);

                for(var i = start;i <= end;i++){
                    this.getImg(this.panels.eq(i));
                }
            },
            ...
            backward : function(e){
                ...
                this.slideTo(cur);
                var index = Math.max(cur,minp);
                index = cur < 0 ? this.panels.length + cur : cur == this.panels.length ? 0 : index;
                this.callback && this.callback(index ,-1);
            },
            forward : function(e){
                if(e&&e.preventDefault){e.preventDefault()}
                var cur = this.curIndex,
