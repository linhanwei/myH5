/**
 * 转盘 抽奖
 * Created by xiuxiu on 2016/2/27.
 */
/*!
 * lottery v1.0.3
 ...
 *     因此，速度值越小，间隔越短，转的越快。
 */
define("plugin/lotterys/1.0.0/lotterys", function() {
    var defaults = {
        selector: '#lottery',
        width: 4,    // 转盘宽度
        height: 4,    // 转盘高度
    ...
    var lottery = {
        ...
        _setup: function () {
            ...
            for (var i = 0; i < this.options.width; ++i) {
                this.container.find('.lottery-group').first().children('.lottery-unit').eq(i).attr('lottery-unit-index', i);
            }
            ...
            for (var i = lottery._count() - this.options.height + 1, j = 0, len = this.options.width + this.options.height - 2; i >= len; --i, ++j) {
                this.container.find('.lottery-group').last().children('.lottery-unit').eq(j).attr('lottery-unit-index', i);
            }

            // 初始化两侧lottery-group的序列
            for (var i = 1, len = this.options.height - 2; i <= len; ++i) {
                this.container.find('.lottery-group').eq(i).children('.lottery-unit').first().attr('lottery-unit-index', lottery._count() - i);
                this.container.find('.lottery-group').eq(i).children('.lottery-unit').last().attr('lottery-unit-index', this.options.width + i - 1);
            }
            this._enable();
        },
        ...
        // 开启抽奖
        _enable: function () {
            this.container.find('a').bind('tap', this.beforeRoll);
        },

        // 禁用抽奖
        _disable: function () {
            this.container.find('a').unbind('tap', this.beforeRoll);
        },
        ...
        beforeRoll: function () {
            var _this = lottery;
            _this._disable();
            if (_this.options.times > 0 && _this.options.beforeRoll) {
                _this.options.beforeRoll.call(_this);
                _this._roll();
            } else {
                //alert('你没有抽奖机会了，赶快去邀请好友获得更多抽奖机会吧！');
                $('body').append('<section class="telDialog wl-trans-dialog translate-viewport" style="display: block;"><div class="cont bounceIn"><p>你没有抽奖机会了，赶快去邀请好友获得更多抽奖机会吧！</p><div class="btngroup"><a class="btn reset">取消</a> <a href="http://miku.unesmall.com/api/h/1.0/inviteAlly.htm" class="btn ring">邀请好友</a></div></div></section>');
                _this._enable();
            }
        },
