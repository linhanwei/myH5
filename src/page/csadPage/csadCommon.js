...
define('h5/js/page/csadCommon', [
    ...
    //框架
    function layout(){
        var template = '<header id="header">MIKU MINE<span id="logout">退出</span> </header><div class="container grid"><div class="leftNav"><div class="headPicCol"><div id="headerimg" class="titleHeader row "><div class="img-circle-50"><img id="myPic"></div><div id="login_user" class="login_user_title col"><p> 您好，<a class="curName"></a></p><p class="cur-online">Online</p></div></div></div><ul></ul></div><div class="rightContainer"> <div class="wrap"><div class="call_title" ><p>暂时没有用户呼叫！</p><ul id="call_title"></ul></div><div class="wrapper"></div> </div> </div></div>';
        ...
        $('.wrapper').height($(window).height()-$('.call_title').height()-$('#header').height()-6);
    }
            ...
            data = [
                {
                    ...
                    url:URL.csadCallCenterPage

                },{
                    name:'聊天消息',
                    className:'chatMessage',
                    url:URL.csadChatMessagePage
                },{
                    name:'管理列表',
                    ...
                    url:URL.csadSalesTrackingPage
                },{
                    name:'蜜月回访',
                    className:'payVisit',
                    url:URL.csadHoneymoonReturnPage
                }],
            tpl ='<li class="{{className}}" href="{{url}}">{{name}}</li>';
...
//如果当前没有某一个联系人的聊天窗口div就新建一个
    ...
    var getChatRecord = function(chatUserId){
        var db = getCurrentDb(),
            //sql = 'delete from chatRecord where rowid = 8 ';
            sql = 'select * from chatRecord where (user_id = "'+chatUserId+'" )';
            //sql = 'select * from chatRecord where rowid = 22';
       // console.log(sql)
            ...
            trans.executeSql(sql, [], function (ts, data) {
                if (data) {
                            for (var i = 0; i < data.rows.length; i++) {
                                ...
                                if(msgtext && msgtext[0].type == 'emotion'){
                                    msgtext = msgLocal;
                                }
                                console.log(msgtext);
                                ...
                                if(from == curUserId){ //发送
                                    appendMsg(curUserId, to, msgtext,'','',time);
                                }
                                else{  //接收

                                    appendMsg(from, from, msgtext,'','',time);
                                }
...
//表情选择div的关闭方法
    ...
    var sendText = function(msg) {
        ...
        var  opts = {
            ...
            messageJ = JSON.stringify(opts);
        insertdbData(to,timestamp,1,1,messageJ);
    ...
    var closeTalking = function(msg) {
        ...
        var db = getCurrentDb();
        db.transaction(function (trans) {
            trans.executeSql("update chatRecord set status = 2 where user_id='"+to+"'", [], function (ts, data) {
            }, function (ts, message) {
                console.log(ts,message);
            });
        });
    ...
    var bigMotion = function(imgSrc,i) {
        ...
        var nowDate = new Date(),
            timestamp = nowDate.getTime();
        var messageJ = JSON.stringify(options);
        insertdbData(to,timestamp,1,1,messageJ);
...
//发送图片消息时调用方法
    var sendPic = function() {
        ...
        if (!Easemob.im.Helper.isCanUploadFileAsync || filetype in pictype) {
            ...
            //var nowDate = new Date(),
            //    timestamp = nowDate.getTime();
            //var messageJ = JSON.stringify(opt);
            //insertdbData(timestamp,1,1,messageJ);
...
//easemobwebim-sdk收到文本消息的回调方法的实现
    var handleTextMessage = function(message) {
        ...
        if(message.ext && message.ext.MikuExpand == 'CallExpert'){
            var db = getCurrentDb();
            db.transaction(function (trans) {
                trans.executeSql("update chatRecord set status = 0 where user_id='"+from+"'", [], function (ts, data) {
                }, function (ts, message) {
                    console.log(ts,message);
                });
            });
    ...
    var showMessageTips = function(who,message){
        ...
        lielem.onclick = function() {
            ...
            URL.assign(URL.csadCallCenterPage);//跳转到呼叫中心页面
                ...
                $.each(membersList,function(i,item){
                    $.each(item.friendsGroupMapList,function(j,memberItem){
                        if(memberItem.emUserName == who){
                            ...
                            strangerIMSingleMsg = [who,memberItem.profilePic,memberItem.profileName,'0'];
                            if(who != null) {
                                strangerIMCount ++;
                                strangerIMGroup[strangerIMCount - 1] = strangerIMSingleMsg;
                                var strangerIMGroupData = JSON.stringify(strangerIMGroup);
        ...
        var callInNum = $('#call_title li').length;

        var num = contactNav == '.chatMessage' ? chatListArr.length : callInNum;
        ...
        var nowDate = new Date(),
         ...
         //   messageC,mestype,messageContent,
        // msgscret='',
          //  msglength=0;
        //if(mestype == 'pic' || mestype == 'aduio'){
          //  msgscret = message.secret ? message.secret : '';
          //  msglength = message.length ? message.length : '';
        //}else{a instanceof Array
        //if(message.data){
        //    if(message.data instanceof Array){
        //        messageC = message.data[0];
        //        mestype = messageC.type;//消息发送的类型
        //        messageContent = messageC.data;//文本消息体
        //    }
        //    else{
        ...
        insertdbData(from,timestamp,0,1,messageJ);
...
//显示聊天记录的统一处理方法
    var appendMsg = function(who, contact, message, onlyPrompt,messExcept,_timestamp) {
        ...
        var chatTime = _timestamp ?  bainx.formatDate('Y-m-d h:i:s', new Date(_timestamp)) : getLoacalTimeString(),
            headstr =(onlyPrompt && !messExcept) ? ["<p1>" + message + "</p1>"] : [ "<p2 class='chatTime'>" + chatTime + "<b></b><br/></p2>","<p1 class='userPhoto'><img src='" + userPhotoSrc + "' /></p1>"];
    ...
    //************************************h5 本地数据库
    initDatabase();
    //insertdbData(0,0,0,2,3,'');
    //deleteTable();

    function insertdbData(to,timestamp,hasRead,isStatus,msg){
        //执行sql脚本，插入数据
        var db = getCurrentDb();
        db.transaction(function (trans) {
            trans.executeSql("insert into chatRecord(user_id,chat_timestamp,read,status,msgbody) values(?,?,?,?,?) ", [to,timestamp, hasRead,isStatus,msg], function (ts, data) {
            }, function (ts, message) {
                 console.log(ts,message);
            });
        });
    ...
    function initDatabase() {
        ...
        db.transaction(function (trans) {//启动一个事务，并设置回调函数
            ...
            trans.executeSql("create table if not exists chatRecord(user_id text null,chat_timestamp int null,read int null,status int null,msgbody text null)", [], function (trans, result) {
